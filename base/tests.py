...
class BaseViewWithNewUserTestCase(TestCase):
    def setUp(self):
        ...
        model_.save()
        # Connections avec le nouvel utilisateur
    ...
    def test_detail_model(self):
        ...
        detail_model3d_url = reverse('base:detail_model', kwargs={'pk': model.id})

        response_detail = self.client.get(detail_model3d_url)
        self.assertEqual(response_detail.status_code, 200)
        self.assertIn('model', response_detail.context)
        self.assertEqual(response_detail.context['model'].name, 'Model with high views')
    ...
    def test_award_start_badge(self):
        model = Model3D.objects.first()
        detail_url = reverse('base:detail_model', kwargs={'pk': model.id})
